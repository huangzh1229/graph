...
struct Graph{
	int n, m;
	vector<int> V;
	vector< map< int, int > > E, E_next;
	vector< vector< pair<int, int> > > Edge, Edge_next;
	vector<int> D;
	Graph(){
		n = m = 0;
		V.clear();
	...
	//	printf("n m: %d %d\n", n, m);
		...
		//	printf("x y z: %d %d %d\n", x, y, z);
			...
			if (E[x].find(y) != E[x].end()){
                ...
                E_next[x].insert(make_pair(y, y));
                E_next[y].insert(make_pair(x, x));
			}
		}
		D.clear();
		D.push_back(0);
	...
	void EdgeInitialize(){
		...
		for (int i = 0; i <= n; i++){
			...
			Edge.push_back(Ed);
            Ed.clear();
            for (map<int, int>::iterator it = E_next[i].begin(); it != E_next[i].end(); it++){
                Ed.push_back(*it);
            }
            Edge_next.push_back(Ed);
		}
	}
	...
	void insertEdge(int u, int v, int k, int next1, int next2){
		if (E[u].find(v) != E[u].end()) return;
		E[u].insert(make_pair(v, k));
		E[v].insert(make_pair(u, k));
        E_next[u].insert(make_pair(v, next1));
        E_next[v].insert(make_pair(u, next2));
        if (next1 == 0|| next2 == 0){
            printf("u, v, k, next1, next2: %d %d %d %d %d\n", u, v, k, next1, next2);
            while (1);
        }
		D[u]++;
		D[v]++;
	...
	void deleteEdge(int u, int v){
     ...
     //   printf("--\n");
        E_next[u].erase(E_next[u].find(v));
     //   printf("--\n");
        E_next[v].erase(E_next[v].find(u));
      //  printf("--\n");
		D[u]--;
		D[v]--;
	}
...
struct Node{
	vector<int> vert, VL, pos, pos2, dis, next, dis_next;
	vector<int> ch;
	int height;
	int pa;
	int uniqueVertex;
...
struct Tree_Decomposition{
	...
	vector< vector<int> > neighbor, length, next;
	vector<int> ord;
	int heightMax;
	void reduce(){
    //    printf("reduce\n");
	...
	//			cout << cnt << endl;
			...
			deg.erase(deg.begin());
			exist[x] = false;
			vector<int> neigh, leng, ne;
			neigh.clear();
			leng.clear();
            ne.clear();
     //   printf("initialize ne\n");
			for (map<int,int>::iterator it = G.E[x].begin(); it != G.E[x].end(); it++){
				int y = (*it).first;
				if (exist[y]){
					neigh.push_back(y);
					leng.push_back((*it).second);
                    ne.push_back(G.E_next[x][y]);
        if (ne[ne.size() - 1] == 0){
            printf("x, y: %d %d\n", x, y);
            if (G.E_next[x].find(y) == G.E_next[x].end())
                printf("not found\n");
            else printf("found\n");
                        while (1);
        }
					...
					if (pu != pv){
						...
						if (G.isEdgeExist(u, v)){
                            if (G.E[u][v] > leng[pu] + leng[pv]){
                                ...
                                G.E_next[u][v] = G.E_next[u][x];
                            }
                            if (G.E[v][u] > leng[pu] + leng[pv]){
								G.E[v][u] = leng[pu] + leng[pv];
                                G.E_next[v][u] = G.E_next[v][x];
                            }
						}
						else {
						//	deg.erase(SelEle(u));
						//	deg.erase(SelEle(v));
        if (G.E_next[u][x] == 0 || G.E_next[v][x] == 0){
            if (G.E_next[u].find(x) == G.E_next[u].end())
        printf("not found\n");
            else printf("found\n");
        if (G.E_next[v].find(x) == G.E_next[v].end())
        printf("not found\n");
            else printf("found\n");
        while(1);
        }
							G.insertEdge(u, v, leng[pu] + leng[pv], G.E_next[u][x], G.E_next[v][x]);
							_DD[u] = G.D[u];
							_DD[v] = G.D[v];
							++_DD2[u];
							++_DD2[v];
...
//		result += pre;
	...
	//	cout << endl << cnt << endl;
		...
		Tree[p].dis.resize(list.size() + 1);
        Tree[p].dis_next.resize(list.size() + 1);
	//	printf("step1");
	...
	//	cout << endl;
		for (int i = 0; i < Tree[p].vert.size(); i++){
            if (Tree[p].dis[toList[Tree[p].vert[i]]] > Tree[p].VL[i]){
                ...
                Tree[p].dis_next[toList[Tree[p].vert[i]]] = Tree[p].next[i];
            }
			int x = Tree[p].vert[i];
			int k;
			...
			for (int j = 0; j < list.size(); j++){
                ...
                if (Tree[p].dis[toList[y]] > z + Tree[p].dis[toList[Tree[p].vert[i]]]){
                    ...
                    Tree[p].dis_next[toList[y]] = Tree[p].next[i];
                }
			}
		}
	//	ct += clock() - t;
	...
	//	printf("step5");
        ...
        printIntVector(Tree[p].dis_next);
      //  cout << Tree[p].dis.size() <<endl;
     //   cout << Tree[p].dis_next.size() <<endl;
	...
	//	cout << endl;
        ...
        printIntVector(Tree[root].dis_next);
		cnt = 0;
		for (int i = 0; i < Tree[root].ch.size(); i++)
	...
	void print2(){
		...
		for (int i = 0; i < Tree.size(); i++){
            ...
            printIntVector(Tree[i].dis_next);
		}
	}
};
